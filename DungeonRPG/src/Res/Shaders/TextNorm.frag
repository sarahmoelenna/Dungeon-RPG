...
void main()
{
	...
	bool blue = false;
	for (int i=0;i<4;i++){
		Shad = (texture2D( ShadowMap, shadowPos.xy + poissonDisk[i]/14000.0)).xyz;
    ...
    //gl_FragColor = vec4((texture2D(ColourMap, gl_TexCoord[0].st)).xyz, 1) * vec4(vec3((FinalLight,FinalLight,FinalLight) + vec3(varyingColour)), 1);
 	...
 	gl_FragData[0] = vec4((texture2D(ColourMap, gl_TexCoord[0].st)).xyz, 1);
   	...
   	gl_FragData[5] = vec4((texture2D(ColourMap, gl_TexCoord[0].st)).xyz, 1) * texture2D(GlowMap, gl_TexCoord[0].st);
 
 	//gl_FragColor = (texture2D( ShadowMap, shadowPos.xy));
 	
 	//gl_FragColor = (NDiff, 1);
